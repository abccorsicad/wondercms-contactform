...
function contactfCSS($args) {
    ...
    $script = '<link rel="stylesheet" href="'.$Wcms->url("plugins/contact-form-my/css/style.css").'" type="text/css">';
    $args[0].=$script;
    return $args;
}
...
function contactfCONTENT() {
    ...
    if(isset($_POST['submitForm']) && ($_SERVER['REQUEST_METHOD'] == "POST")) {
       ...
       if(empty($antispam) || ($parola1!==$antispam) ) {
            $aFout[] = $i18n['antispam_error'];
            unset($antispam);
            $fout['text']['antispam'] = TRUE;
            $fout['input']['antispam'] = TRUE;
        }
    ...
    if($formulier) {
		...
		$parole=array("vero","oggi","informazioni","umano","giallo","verde","posta"); $parola=$parole[rand(0,count($parole)-1)];
		$final_content .="Per dimostrare di essere umano, e prevenire lo spam, digita qui sotto la parola <span style=\"font-weight:bold;font-style: italic;\">" . $parola ."</span>";
		$final_content .="<input type='hidden' placeholder='$i18n[parola1]' id='parola1' name='parola1' value='". $parola ."'/>";

        $final_content .=  "<div class='form-group'><input type='text' placeholder='$i18n[antispam]' id='antispam' name='antispam' maxlength='40'";
        if(isset($fout['input']['antispam'])) { $final_content .=  "class='fout'"; } $final_content .=  "value='";
        if (!empty($antispam)) { $final_content .=  stripslashes($antispam); } $final_content .=  "' /></div>";
